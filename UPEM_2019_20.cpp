	...
	HestonModel model_Heston)
{
	...
	for (int test_index = 0; test_index < number_of_tests; ++test_index)
	{
		double pv_vol = pricer_Heston_var->price();

		std::cout << "Var swap closed formula price with Heston model for test number " << test_index << " is " << pv_vol << "\n";
		std::cout << "" << "\n";
	}
	std::cout << "\n";
}
...
int main()
{
	...
	size_t number_of_simulations = 10000; //5k
